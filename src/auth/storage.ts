...
import { User } from "../http-client/interfaces/user.interface.js";

const SERVICE_NAME = "workos-cli";
const ACCOUNT_NAME = "default";
export const WORKOS_DIR =
  process.env.WORKOS_TOKEN_DIR || join(homedir(), ".workos");
...
export async function saveAuthData(
  data: AuthData
): Promise<{ tokenPath: string; dirCreated: boolean }> {
  try {
    ...
    console.warn(
      chalk.yellow(
        "Failed to store credentials in system keychain, falling back to file storage"
      )
    );
...
export async function getAuthData(): Promise<AuthData | null> {
  try {
    ...
    console.warn(
      chalk.yellow(
        "Failed to retrieve credentials from system keychain, trying file storage"
      )
    );
    console.warn(error instanceof Error ? error.message : String(error));
  }
